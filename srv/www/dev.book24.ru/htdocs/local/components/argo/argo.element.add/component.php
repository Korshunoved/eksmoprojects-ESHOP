...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["save"]) || !empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
			...
			else
			{
				...
				if (!$arParams["ID"] = $oElement->Add($arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
				{
					$arResult["ERRORS"][] = $oElement->LAST_ERROR;
                                ...
                                {
                                    file_put_contents($_SERVER['DOCUMENT_ROOT'].'/logF.txt', var_export($arUpdateValues, true), FILE_APPEND);
        /*if (!$ORDER_ID = CSaleOrder::Add(array(
                                                "LID"              => SITE_ID,
                                                "PERSON_TYPE_ID"   => 3,
                                                "PAYED"            => "N",
                                                "CANCELED"         => "N",
                                                "STATUS_ID"        => ($arUpdateValues['PROPERTY_VALUES'][221]) ? "PO" : '',
                                                "PRICE"            => 0,
                                                "CURRENCY"         => 'RUB',
                                                "USER_ID"          => 3,
                                                "PAY_SYSTEM_ID"    => ($arUpdateValues['PROPERTY_VALUES'][203]) ? $paysys[$arUpdateValues['PROPERTY_VALUES'][203]] : $paysys[$arUpdateValues['PROPERTY_VALUES'][212]],
                                                "PRICE_DELIVERY"   => 0,
                                                "DELIVERY_ID"      => ($arUpdateValues['PROPERTY_VALUES'][202]) ? $delsys[$arUpdateValues['PROPERTY_VALUES'][202]] : $paysys[$arUpdateValues['PROPERTY_VALUES'][211]],
                                                "DISCOUNT_VALUE"   => 0,
                                                "TAX_VALUE"        => 0.0,
                                                "USER_DESCRIPTION" => ''
                                        ))
        ...
        if (!Add2BasketByProductID($arUpdateValues['PROPERTY_VALUES'][195], 1, array(
                    'ORDER_ID' => $ORDER_ID,
                        ), $basket_props)) {
