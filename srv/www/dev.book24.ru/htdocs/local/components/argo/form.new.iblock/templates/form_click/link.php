...
<script>
	BX.ready(function(){
            window.Book24 = {};
		window.Book24.openForm = function (params) {
			...
			var popupParams =  {
				...
				events: {
					onPopupClose : function(popupWindow){
						popupWindow.destroy();
						window.Book24.activePopup = null;
					}
				}
			};
			if(params.title)
			...
			BX.ajax({
				url:'<?=$this->__component->getPath()?>/ajax/form_click.php',
				...
				onsuccess: function(data){
					...
					formPopup.show();
					window.Book24.activePopup = formPopup;
				}
			});

		};
		window.Book24.showFormSuccess = function(formId, data){
		...
		window.Book24.formRefresh = function (formId) {
			BX.ajax.submit(BX("iblockForm" + formId),function(data){
				data = data.replace(/<div[^>]+>/gi, '');//strip_tags
				data.substr(0,7);
				if(data.substr(0,7) === 'success')
				{
					window.Book24.showFormSuccess(formId,data)
					return false;
				}
				BX('ajaxFormContainer_' + formId).innerHTML = data;
			});
