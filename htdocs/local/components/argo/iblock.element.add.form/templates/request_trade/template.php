...
<form id="iblockForm<?=$arParams["IBLOCK_ID"]?>" name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
			...
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
					...
					[<a href="<?=$arResult["ELEMENT_FILES"][$value]["SRC"]?>"><?=GetMessage("IBLOCK_FORM_FILE_DOWNLOAD")?></a>]<br />
							...
							<input type="<?=$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
									...
									case "multiselect":
									?>
							<select <?echo ($inputType=='hidden') ? 'style="display:none;"' : '';?> name="PROPERTY[<?=$propertyID?>]<?=$type=="multiselect" ? "[]\" size=\"".$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]."\" multiple=\"multiple" : ""?>">
	...
	<input onclick="Argo.formRefresh(<?=$arParams["IBLOCK_ID"]?>);return false;" <?if($arParams["HIDE_SUBMIT"] == 'Y') echo 'style="display: none;"'?> class="color-button bxr-color bxr-color-button bxr-bg-hover-light" id="submitForm_<?=$arParams["IBLOCK_ID"]?>" type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
...
<script>
    $(function() {
       ...
       $('input[data-code="USER_ID_HIDDEN"]').val(user_id);
    });
</script>